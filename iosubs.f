...
C           Physical record isn't in buffer, get it.
	    read(iucd, rec=nprec, iostat=ios) inbuf(1:lrecl)
	    if (ios.lt.0) go to 99
	    if (ios.gt.0) go to 98
	    recno = nprec
	 endif
...
C           and continuation indicator.
	 ...
	 if (inbuf(1+noffs+pos(2):6+noffs+pos(2)) .ne. ' ') go to 5
...
C           Check validity of new logical record.
     ...
     &          0 .eq. index('VAST',lrtype)) then
               write(0,*) '**GETBKT:  Record ',lrec,' (expecting ',
     &           i,') type ',lrtype,' continuation ',lrcc,' invalid.'
...
C        Physical record isn't in buffer, get it.
     ...
     &      lrec, lrtype, lrcc
      if (lrec .ne. block .or. 
     &    lrtype .ne. 'D' .or. lrcc .ne. ' ') go to 98
